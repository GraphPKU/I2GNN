...
import numpy as np
from gatedgcn import HeteroGatedGraphConv
...
class Nested2_k1_GNN(torch.nn.Module):
    def __init__(self, dataset, num_layers=5, subgraph_pooling='mean', use_pos=False,
        ...
        super(Nested2_k1_GNN, self).__init__()
        ...
        self.convs.append(GatedGraphConv(M_out, 1))
        self.z_embedding_list.append(torch.nn.Embedding(1000, M_in))
...
class IDGNN(torch.nn.Module):
    def __init__(self, dataset, num_layers=5, subgraph_pooling='mean', use_pos=False,
        ...
        self.subgraph_pooling = subgraph_pooling
        self.y_ndim = y_ndim
        self.node_embedding = torch.nn.Embedding(1, 64)
        self.convs = torch.nn.ModuleList()
        M_in, M_out = 64, 64
        # M_in, M_out = dataset.num_features + 8, 1
        # nn = Sequential(Linear(edge_attr_dim, 128), ReLU(), Linear(128, M_in * M_out))
...
class PPGN(torch.nn.Module):
    # Provably powerful graph networks
    def __init__(self, dataset, emb_dim=300, use_embedding=False, use_spd=False, y_ndim=1,
                 **kwargs):
